...
const MealPlanner = () => {
  ...
  useEffect(() => {
    const fetchSavedRecipes = async () => {
      if (!user) return;
      try {
        const response = await axios.get(`${process.env.PUBLIC_URL}/api/recipes/saved/${user.uid}`);
  ...
  const handleGenerateMealPlan = async () => {
    ...
    try {
      const response = await axios.post(`${process.env.PUBLIC_URL}/api/meal-planner/generate`, {
        savedRecipes,
